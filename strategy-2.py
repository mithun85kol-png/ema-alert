...
def compute_consolidation_window(history):
    """
    Consolidation Breakout (added, per request — "Consolidation dhorte
    parbe?") — checks whether the trailing config.CONSOLIDATION_
    LOOKBACK_DAYS COMPLETED trading days (from `history`, which already
    excludes today — see main.fetch_daily_history) have been trading in
    a TIGHT range, i.e. "coiling" before a potential breakout.

    Tight means (highest HIGH - lowest LOW) over the window, as a % of
    the window's average CLOSE, is <= config.CONSOLIDATION_MAX_RANGE_PCT.
    A wide/trending window (not actually consolidating) returns None —
    there's nothing meaningful to break out of.

    Returns None if there isn't enough history yet, or the window
    wasn't actually tight. Otherwise:
      {"range_low": float, "range_high": float, "range_pct": float,
       "avg_volume": float, "days": int}
    range_low/range_high are the window's lowest LOW / highest HIGH —
    the actual breakout levels a later close needs to clear. avg_volume
    is the window's average daily volume, the baseline a breakout
    candle's volume is compared against (see
    check_consolidation_breakout_live / check_consolidation_breakout_scan
    below).
    """
    lookback = config.CONSOLIDATION_LOOKBACK_DAYS
    if len(history) < lookback:
        return None

    window = history[-lookback:]
    range_high = max(day["high"] for day in window)
    range_low = min(day["low"] for day in window)
    avg_close = sum(day["close"] for day in window) / len(window)
    if avg_close <= 0:
        return None

    range_pct = (range_high - range_low) / avg_close * 100
    if range_pct > config.CONSOLIDATION_MAX_RANGE_PCT:
        return None

    avg_volume = sum(day["volume"] for day in window) / len(window)
...
def _consolidation_breakout_from(symbol, close, volume, cw):
    """Shared breakout-decision logic for the live and EOD checks
    below — both just supply `close`/`volume` from a different source
    (today-so-far vs. today's finalized daily bar) and get back the
    same signal shape."""
    if not cw:
        return None

    volume_multiple = round(volume / cw["avg_volume"], 2) if cw["avg_volume"] else 0
    if volume_multiple < config.CONSOLIDATION_BREAKOUT_MIN_VOLUME_MULTIPLE:
        return None

    if close > cw["range_high"]:
        direction = "BULLISH"
    elif close < cw["range_low"]:
        direction = "BEARISH"
    else:
        return None
    ...
    return {
        ...
        "direction": direction,
        "close": close,
        "range_low": cw["range_low"],
        "range_high": cw["range_high"],
        "range_pct": cw["range_pct"],
        "volume_multiple": volume_multiple,
    }


def check_consolidation_breakout_live(symbol, current_close, current_volume_so_far, cw):
    """
    LIVE (intraday) Consolidation Breakout check (added, per request —
    "emergency alert e add kora jay na", i.e. check this every scan
    cycle in the main real-time loop, not just once/day at close). `cw`
    is the consolidation window from compute_consolidation_window
    (already computed once/day from cached daily history — see
    main.build_momentum_volume_data). Fires the moment TODAY's current
    price/volume-so-far breaks the window, without waiting for today's
    candle to close.

    NOTE: current_volume_so_far is only PART of today's eventual
    full-day volume (however much of the session has elapsed so far),
    while cw["avg_volume"] is a FULL-DAY average from the consolidation
    window — so the volume_multiple this produces UNDERSTATES the
    likely end-of-day multiple, especially early in the session. This
    is a deliberate simplification (no time-of-day elapsed-fraction
    scaling) — treat an early-session volume_multiple as a floor, not
    the final number; it only gets more accurate as the day progresses.

    Returns None if cw is None, volume confirmation fails, or price
    hasn't broken either side of the range yet. Otherwise a signal
    dict: {"symbol", "direction" ("BULLISH"/"BEARISH"), "close",
    "range_low", "range_high", "range_pct", "volume_multiple"} — same
    shape check_consolidation_breakout_scan returns, so both feed
    telegram_notifier.send_consolidation_breakout_summary identically.
    """
    return _consolidation_breakout_from(symbol, current_close, current_volume_so_far, cw)


def check_consolidation_breakout_scan(history, today_bar, symbol):
    """
    EOD (once/day, after close) Consolidation Breakout check (added,
    per request). Computes the consolidation window from `history`
    (pre-today daily candles — see main.fetch_daily_history) via
    compute_consolidation_window, then checks whether TODAY's own close
    (today_bar, from main.build_todays_daily_bar) broke out of it, with
    volume confirmation.

    Unlike check_consolidation_breakout_live, today_bar["volume"] is
    genuinely today's FULL session volume (this scan runs at/after
    market close), so this volume_multiple is accurate, not a
    lower-bound estimate.

    Returns None if there's no consolidation window (not enough
    history, or the window wasn't actually tight), volume confirmation
    fails, or today's close didn't break either side. Otherwise the
    same signal shape as check_consolidation_breakout_live.
    """
    cw = compute_consolidation_window(history)
    return _consolidation_breakout_from(symbol, today_bar["close"], today_bar["volume"], cw)
...
def compute_intraday_checklist(signal):
    ...
    CHANGED (per request, 2026-08-27 — "1st 15 minit candle open =
    low/high sorao... smart money-r momentum ar volume spike-ta add
    kore score generate korle kemon hoy"): the old "1st 15-min candle
    Open = Low/High" item (2 pts) is REMOVED — it's a static fact
    fixed at 09:30 that stays the same all day, so on a cross that
    fires hours later (this is now the 15-min primary timeframe) it
    was really just re-scoring "how the morning opened," not this
    candle's setup. In its place, two fields already computed for
    Smart Money are folded in here too (1 pt each): Momentum and
    Volume Spike. Total is still a fixed /10.

    Mirrors automatically for BULLISH (BUY setup) vs BEARISH (SELL
    setup) signals -- same checks, opposite direction each time:
      ...
      RSI > 50                +1             RSI < 50                +1
      Momentum (4-wk high)    +1             No fresh 4-wk high      +1
      Volume Spike (vs 5d ago)+1             Volume Spike (vs 5d ago)+1
      1st 15-min High breakout+2             1st 15-min Low breakdown+2

    Momentum note: signal["momentum"] only ever means "today's close
    is above the trailing 4-week high" (see main.py's
    build_momentum_volume_data) -- there's no separate "4-week low"
    tracked for a bearish mirror. So for a BEARISH signal this check
    instead rewards momentum being False, i.e. price is NOT currently
    making a fresh high -- read it as "no fresh breakout strength
    working against this short," not a true bearish-momentum measure.
    If that framing doesn't feel right for how you trade shorts, this
    is the one item worth reconsidering / dropping back out.

    Volume Spike (yesterday's volume vs. 5 trading days before that)
    is direction-agnostic on purpose -- it flags unusual volume
    interest in the stock generally, which is treated as supportive
    context either direction, same as the "Crossing vol > prev" /
    "Volume > recent avg" checks already here.

    This is NOT rescaled by how many dimensions had data -- every
    check always counts (missing/unavailable data just means that
    check is simply not satisfied, i.e. 0 points for it, same as a
    real "no"), so the denominator is always a fixed /10 and directly
    comparable across every alert.

    Reads only fields the signal already carries (from _evaluate_candle,
    main.py's opening_candle_bias/momentum/volume_spike attachment) --
    no extra fetch. IMPORTANT: momentum/volume_spike must be attached
    to the signal BEFORE this is called (main.py now does this).
    ...
    momentum = signal.get("momentum")
    momentum_ok = momentum is not None and (momentum if bullish else not momentum)

    volume_spike = signal.get("volume_spike")
    volume_spike_ok = volume_spike is True
    ...
    items = [
        ...
        ("Momentum: above 4-week high" if bullish else "No fresh 4-week high (momentum not against short)", momentum_ok, 1),
        ("Volume Spike (vs. 5 trading days ago)", volume_spike_ok, 1),
...
def passes_alert_gate(signal):
    """
    Alert Gate (SIMPLIFIED, per request, 2026-08-28 — "EMA CROSS WITH
    GOOD/STRONG TRADING SCORE") — a single condition. Sends the alert
    if, and only if, Trading Score is GOOD or STRONG:
      trading_score.score >= config.QUALITY_GATE_MIN_TRADING_SCORE
    (label bands: 8-10 STRONG, 6-7.9 GOOD, 4-5.9 MODERATE, <4 WEAK —
    see compute_trading_score). Trading Score already rolls up Daily
    Score, Buy/Sell Score, Smart Money, and Near-High into one number,
    so this single check effectively reflects all of them together —
    the previous 6-case OR-gate (Daily Score alone, Buy/Sell Score
    alone, Smart Money alone, a "combined" case, Bulk/Block deal,
    Volume Spike %) is REMOVED from the gate decision. Those fields
    are still computed and still shown on every alert message — only
    the gate's own pass/fail logic changed.

    Requires trading_score to already be attached to the signal.

    Returns (passed: bool, reasons: list[str]) — reasons has at most
    one entry, for logging / the "why this fired" line on the alert.
    """
    trading = signal.get("trading_score")
