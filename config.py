...
DAILY_SCORE_REPORT_ENABLED = False  # CHANGED (per request, 2026-08-31 — "ei duto alert sorao" — Perfect Daily Score alert turned off)
DAILY_SCORE_REPORT_MIN_SCORE = 8   # out of 8 — score must be >= this
DAILY_SCORE_REPORT_STATE_FILE = "daily_score_report_state.json"

# Trading Score Summary roll-up message — ADDED flag (per request,
# 2026-08-31 — "ei duto alert sorao"), turned off. This was the combined
# "🏆 Trading Score Summary — GOOD/STRONG" message (see main.py, near
# send_trading_score_summary) listing every stock whose individual
# EMA-cross alert fired that run. Each stock's own separate alert is
# unaffected — this only controls the extra roll-up message.
TRADING_SCORE_SUMMARY_ENABLED = False
...
# Market-wide (every symbol, same universe as the NSE Bulk Deals page,
# NOT limited to the F&O/Nifty 500 watchlist) — fires once per NEW
# deal found in NSE's bulk.csv/block.csv archive snapshot. See
# bulk_block_data.check_and_alert / get_all_recent_deals.
BULK_DEAL_ALERT_ENABLED = False  # CHANGED (per request, 2026-08-30 — "bulk deal notification stop koro")
BULK_DEAL_ALERT_STATE_FILE = "bulk_deal_alert_state.json"
# How many days of "already seen" deal ids to keep in the state file
# before pruning — NSE's own CSVs only hold a short trailing window
# anyway, so this just needs to comfortably exceed that window.
BULK_DEAL_ALERT_LOOKBACK_DAYS = 10
...
# ---------- Consolidation Breakout (added, per request —
# "Consolidation dhorte parbe?") ----------
# A stock is "consolidating" when its trailing CONSOLIDATION_LOOKBACK_
# DAYS daily range (highest HIGH - lowest LOW) is tight relative to its
# average close — see strategy.compute_consolidation_window. A
# breakout fires when price later closes outside that range WITH
# volume confirmation (>= CONSOLIDATION_BREAKOUT_MIN_VOLUME_MULTIPLE x
# the window's own average daily volume) — see
# strategy.check_consolidation_breakout_live (checked every scan cycle
# in the main real-time loop, using today's volume-so-far) and
# strategy.check_consolidation_breakout_scan (the standalone once/day
# EOD version, SCAN_MODE=consolidation_breakout_scan, using today's
# finalized full-day volume).
...
# run_top_movers_1min / run() dispatch) — meant to run via its own
# cron-job.org trigger a minute or two after market open (09:15 IST),
# once today's very first 1-minute candle (09:15:00-09:16:00) has
# actually closed and is available from Upstox. F&O-stock universe
# only (same as the Opening 15-min Bias report), ranked by % change
# of that first 1-min candle's CLOSE vs the previous trading day's
