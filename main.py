        ...
        try:
            ...
            for signal in signals:
                ...
                try:
                    ...
                    # 2026-08-29) — collect every stock whose alert
                    # actually went out this run (i.e. passed the
                    # EMA-cross gate + Trading Score GOOD/STRONG gate),
                    # for the combined summary message sent once after
                    # this whole scan finishes (see run()).
...
def run():
    ...
    # Trading Score Summary (added, per request, 2026-08-29 — "trading
    # score good/strong hole eksathe stock summary dao with chart") —
    # ONE combined message listing every stock whose individual
    # EMA-cross alert actually went out THIS run (i.e. already passed
    # both the EMA-cross gate and the Trading Score GOOD/STRONG gate —
    # see strategy.passes_alert_gate). This is IN ADDITION to each
    # symbol's own detailed alert, not a replacement — the individual
    # alerts still carry the full Trade Plan/RSI/PCR/etc detail this
    # summary intentionally leaves out. No separate dedup needed: each
    # hit here already represents a freshly-sent, cooldown-cleared
    # alert this exact run (see state.mark_alerted upstream), so
    # there's nothing to compare against a "last sent" state file —
    # if the list is non-empty, it's genuinely new this run. Simply
    # not sent on a run with zero qualifying alerts (no empty message).
