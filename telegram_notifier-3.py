...
def send_alert(signal):
    ...
    # Alert Gate trigger reasons (added, per request, 2026-08-27) —
    # which of the 6 OR-gate cases (see strategy.passes_alert_gate)
    # actually let this alert through, shown as a short "Trigger:"
    # line right under Trading Score so it's clear at a glance whether
    # this fired on score strength, a Bulk/Block deal, or a volume
    # spike. Empty when the gate is disabled (config.QUALITY_GATE_ENABLED
    # = False) since gate_reasons is never set in that case.
...
def send_bulk_deal_summary(deals, now_ist):
    ...
    Was previously one Telegram message PER deal (send_bulk_deal_alert
    below, still kept for reference/any other caller); every NEW deal
    found this run by bulk_block_data.check_and_alert now goes out
    together in ONE message instead — market-wide, every symbol, same
    as the NSE Bulk Deals page itself, NOT limited to the F&O/Nifty 500
    watchlist the rest of this bot scans.
    ...
    Grouped BUY first then SELL (each group in the caller's existing
    order), numbered within each group.

    Telegram caps a single message at 4096 characters — with enough
    deals in one run (e.g. catching up after downtime) that limit CAN
    be hit, so this splits into multiple messages (each still holding
    as many deals as fit) rather than silently truncating or failing.
    Sends nothing if `deals` is empty.
    """
    if not config.TELEGRAM_BOT_TOKEN or not config.TELEGRAM_CHAT_ID:
    ...
    def _format_deal(i, deal):
        ...
        return (
            f"{i}. {arrow} <b>{deal['symbol']}</b> — {type_icon} {deal_type} ({buy_sell or 'N/A'})\n"
            f"   {deal.get('date', 'N/A')} | {client} | Qty {quantity} | ₹{price}"
        )

    buy_deals = [d for d in deals if d.get("buy_sell") == "BUY"]
    sell_deals = [d for d in deals if d.get("buy_sell") == "SELL"]
    other_deals = [d for d in deals if d.get("buy_sell") not in ("BUY", "SELL")]

    body_lines = []
    if buy_deals:
        body_lines.append(f"🟢⬆️ <b>Buy</b> ({len(buy_deals)}):")
        body_lines.extend(_format_deal(i, d) for i, d in enumerate(buy_deals, start=1))
        body_lines.append("")
    if sell_deals:
        body_lines.append(f"🔴⬇️ <b>Sell</b> ({len(sell_deals)}):")
        body_lines.extend(_format_deal(i, d) for i, d in enumerate(sell_deals, start=1))
        body_lines.append("")
    if other_deals:
        body_lines.append(f"⚪ <b>Other</b> ({len(other_deals)}):")
        body_lines.extend(_format_deal(i, d) for i, d in enumerate(other_deals, start=1))

    header = f"📦🧱 <b>Bulk/Block Deals</b> — {date_str} {time_str} ({len(deals)} new)\n"

    # Chunk into <=4096-char messages (Telegram's own hard cap) —
    # simple greedy line-by-line packing, splitting between lines
    # only, never mid-line.
