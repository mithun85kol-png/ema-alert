...
def passes_alert_gate(signal):
    ...
    for logging / an optional "why this fired" line on the alert.
    """
    reasons = []

    daily = signal.get("daily_score")
    daily_ok = daily is not None and daily["score"] > config.QUALITY_GATE_MIN_DAILY_SCORE
    if daily_ok:
        reasons.append(f"Daily Score {daily['score']}/{daily['total']}")

    checklist = signal.get("intraday_checklist")
    setup_ok = checklist is not None and checklist["score"] > config.QUALITY_GATE_MIN_SETUP_SCORE
    if setup_ok:
        reasons.append(f"Buy/Sell Score {checklist['label']}")
    ...
    if combined_ok:
        reasons.append("Combined (Daily+Buy/Sell+Trading)")

    # ---- Bulk/Block deal, same rule as compute_smart_money_signal ----
    bulk_ok = False
    deal = signal.get("last_bulk_block_deal")
    direction = signal.get("direction")
    if deal and deal.get("date") and signal.get("candle_time") and direction:
        expected_side = "BUY" if direction == "BULLISH" else "SELL"
        if (deal.get("buy_sell") or "").upper() == expected_side:
            deal_dt = _parse_deal_date(deal["date"])
            candle_dt = _parse_candle_time(signal["candle_time"])
    ...
    # ---- Volume Spike % ----
    vs_pct = signal.get("volume_spike_pct")
    vs_ok = vs_pct is not None and vs_pct > config.ALERT_GATE_VOLUME_SPIKE_PCT
    if vs_ok:
