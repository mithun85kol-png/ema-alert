    ...
    for symbol, (df5, df75, df15) in dfs.items():
        try:
            ...
            for signal in signals:
                ...
                # Alert Gate (REPLACED, per request, 2026-08-27) — OR
                # of 6 conditions, ANY one is enough to send. Moved
                # here (was much earlier, AND-only) since it now needs
                # Bulk/Block deal + Smart Money + Trading Score, all
                # computed above by this point. See
                # config.py's "Alert Gate" comment and
                # strategy.passes_alert_gate for the exact 6 cases. Not
                # marked alerted on failure, so it's re-checked next
                # run.
    ...
    for symbol, (df5, df75, df15) in dfs.items():
        ...
        try:
            ...
            for signal in signals:
                ...
                # Alert Gate (REPLACED, per request, 2026-08-27) — see
                # the matching comment in run_fo_scan() above.
                if config.QUALITY_GATE_ENABLED:
                    gate_passed, gate_reasons = passes_alert_gate(signal)
                    if not gate_passed:
                        continue
